...
async def add_new_user(name: str, login: str, password: str, connPool: asyncpg.Pool) -> DbResult:
    ...
    #check if exists
    nameResult: DbResult = get_user_by_name(name, connPool)
    loginResult: DbResult = get_user_by_login(login, connPool)
    ...
    async with connPool.acquire() as conn:
        ...
        conn.execute("INSERT INTO Users(name, login, password) VALUES(:name, :login, :password)", {
            "login": login,
            "name": name,
            "password": password
        })
...
async def get_user_by_name(name: str, connPool: asyncpg.Pool)  -> DbResult:
    ...
    async with connPool.acquire() as conn:
        row = conn.fetchrow("SELECT id, name FROM Users WHERE name=:name LIMIT 1",{"name": name})
...
async def get_user_by_login(login: str, connPool: asyncpg.Pool) -> DbResult:
    ...
    async with connPool.acquire() as conn:
        ...
        row = conn.fetchrow("SELECT id, name FROM Users WHERE login=:login LIMIT 1",{"login": login})
...
async def get_users_preview(limit: int, connPool: asyncpg.Pool)  -> DbResult:
    ...
    async with connPool.acquire() as conn:
        rows = conn.fetch("SELECT id, name FROM Users LIMIT :limit", {"limit": limit})
