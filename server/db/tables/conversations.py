...
async def get_conversation(convId: int, connPool: asyncpg.Pool) -> DbResult:
    ...
    async with connPool.acquire() as conn:
        row = await conn.fetchrow("SELECT title, id FROM conversations WHERE id=:convId LIMIT 1", {
            "convId": convId
        })
...
async def get_conversations(limit: int, connPool: asyncpg.Pool) -> DbResult:
    ...
    async with connPool.acquire() as conn:
        rows = await conn.fetch("SELECT title, id FROM conversations LIMIT :limit", {
            "limit": limit
        })
...
async def get_conversation_by_title(title: str, connPool: asyncpg.Pool) -> DbResult:
    ...
    async with connPool.acquire() as conn:
        row = await conn.fetchrow("SELECT id FROM conversations WHERE title=:title", {
            "title": title
        })
