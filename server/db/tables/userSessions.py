...
async def check_if_token_in_db(token: str, connPool: asyncpg.Pool) -> DbResult:
    ...
    async with connPool.acquire() as conn:
        row = conn.fetchrow("SELECT id FROM UserSessions WHERE token=:token LIMIT 1",{"token": token})
    if (row):
...
async def add_token_to_db(token: str, userId: int,  connPool: asyncpg.Pool) -> DbResult:
    ...
    async with connPool.acquire() as conn:
        conn.execute("INSERT INTO UserSessions(token, userId) VALUES(:token, :userId)",{"token": token, "userId": userId})
...
async def get_token_data(token: str, connPool: asyncpg.Pool) -> DbResult:
    ...
    async with connPool.acquire() as conn:
        row = conn.fetchrow("SELECT userId, id FROM UserSessions WHERE token=:token LIMIT 1",{"token": token})
